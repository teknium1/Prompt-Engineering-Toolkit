            ...
            <Box sx={{ p: 2 }}>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                {modelConfigs.map((config) => (
                  <Box key={config.id} sx={{ flexGrow: 1, flexBasis: 'calc(50% - 16px)', minWidth: 300 }}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                      ...
                      {!useGlobalPrompt ? (
                        config.prompts.map((prompt, index) => (
                          <Box key={prompt.id} sx={{ mb: 2 }}>
                            <Typography variant="h6" gutterBottom>Turn {index + 1}</Typography>
                            {index === 0 && (
                              <TextField
                                fullWidth
                                multiline
                      ...
                      {!useGlobalPrompt && (
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                          ...
                          {config.prompts.length > 1 && (
                            <Button variant="outlined" onClick={() => handleRemoveLastTurn(config.id)} startIcon={<DeleteIcon />}>
                              Remove Last Turn
                            </Button>
                          )}
                        </Box>
                ...
                {modelConfigs.map((config) => (
                  <Box key={config.id} sx={{ mb: 2 }}>
                    ...
                    {config.provider === 'azure' && (
                      <>
                        <TextField fullWidth size="small" label="Resource Name" value={config.resourceName || ''} sx={{ mb: 1 }}
                          onChange={(e) => handleModelConfigChange(config.id, 'resourceName', e.target.value)}
                        />
                        <TextField fullWidth size="small" label="Deployment ID" value={config.deploymentId || ''} sx={{ mb: 1 }}
                    ...
                    {config.provider === 'bedrock' && (
                      <>
                        <TextField fullWidth size="small" label="Region" value={config.region || ''} sx={{ mb: 1 }}
                          onChange={(e) => handleModelConfigChange(config.id, 'region', e.target.value)}
                        />
                        <TextField fullWidth size="small" label="Model ID" value={config.modelId || ''} sx={{ mb: 1 }}
                    ...
                    {savedModels.map((savedModel, index) => (
                      <Box key={index} sx={{ mb: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
                          ...
                          <Button variant="outlined" onClick={() => removeSavedModel(index)} startIcon={<DeleteIcon />}>
                            Remove
                          </Button>
                        </Box>
                      </Box>
